...
class DiffusionLoader(Dataset):
    def __init__(self, root = 'iclevr', json_file = f"json/train.json"):
        ...
        if 'train' in json_file:
            ...
            test = self.Load_Testset(f'json/test.json')
            test2 = self.Load_Testset(f'json/new_test.json')
            self.img = None
    ...
    def Load_Dict(self):
        with open('json/objects.json', 'r') as file:
            data = json.load(file)
        return data
